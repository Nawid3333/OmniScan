...
## 8. Acquisition
> **Superseded 2026-09-22 (card RM1):** the acquisition subsystem was removed; raws are exclusively user-supplied (`omniscan import` / the GUI import page). Kept as historical record.
...
Actions: CPU tests on push (P2), app release builds (U4), `model-watch` (§2). (The `relay-deploy` workflow existed for the removed acquisition subsystem and was deleted with it, card RM1.) Repository visibility (private today) decides whether end users can pull the model mirror and app updates anonymously (owner decision pending).
...
H1 hardware + compatibility · O1a catalog of all OCR models/sizes · O1b OCR engines (ppocr sizes, manga-ocr, PaddleOCR-VL) · O1c qualification suite + model-watch Action · F2 filter tiers + batch · S2 slicer strategies + compare · P1 runner gates and previews · B11' debugger/side-by-side views · R1' reader · L1 library covers/metadata · B33b extract.pics client + chapter selection + completeness checks (built, then removed with the subsystem — RM1) · G1 `ask` helper · then the PySide6 shell (U3) on top of the JSON interfaces.
