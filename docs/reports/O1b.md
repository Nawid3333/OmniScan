...
Rebased on `main`; `ruff format/check` clean, `pyright` 0 errors, full CPU suite **3281 passed**. **Live check with the real model** (`omniscan models download ocr-rec-manga-ocr-2025`, then `OMNISCAN_OCR__ENGINE=manga_ocr OMNISCAN_OCR__LANG=ja omniscan ocr PepperCarrotJA -c "Episode 06" --force`): 91 regions read in **3.8 s** on the RX 9070 XT, `engine = ocr-rec-manga-ocr-2025`, scores 0.89–1.0, texts such as `あれ、しまったまた窓を開けたまま寝ちゃったみたい...` (correct), the stylised title `ペッパー…` misread as `代ッパー…` (expected for logo lettering). `omniscan eval PepperCarrotJA -c "Episode 06" --lang ja`: character recall 0.90, page chrF **0.686**; the *box-level* CER (0.84) is not trustworthy for Japanese yet — the truth-box assignment of the eval was built for the Korean SVGs (to be re-checked in card O1c, page chrF is the primary cross-language metric until then).
**GPU smoke test fixed by the director:** the test rendered one 15:1 text line, which manga-ocr (like every crop reader that squashes to 224×224) cannot read, and none of the bundled fonts has kana, so it now wraps the text to compact blocks, detects missing glyphs by comparing with the "not defined" glyph, and skips when no font can draw the kana (today: skipped, the live check above covers the real path). Answers: (2) yes, `"lines" = len(regions)` stays for crop engines, also in O1d.
