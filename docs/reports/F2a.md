...
Live check on `omniscan filter run PepperCarrotKR -c "Episode 06"` (real GPU, real chapter) crashed in `slice` on `dhash_tensor`: ROCm's `interpolate(..., antialias=True)` overflows its box-filter buffer on a full-page-sized crop (repro'd directly at 3x2934x800 and larger, independent of the pipeline). Sent the reproduction back to the builder; fix (`b83977a`, coarse `avg_pool2d` before the antialiased resize for inputs over 512 px/side, exact-hash-preserving below that) verified: rebased on main, ruff/pyright clean, CPU suite 3562 passed, the new GPU regression test (the exact crashing shape) passed on the RX 9070 XT, and the live check now completes end to end — `filter run` correctly filtered the injected duplicate page (`00.jpg`) with a byte-identical `_filtered/` copy, no slice-level false positive. Live-check artifacts (the added promo example, the `_filtered/` copy) removed afterward.
