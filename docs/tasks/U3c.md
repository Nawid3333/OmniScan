...
4. A real end-to-end check on this PC: `omniscan gui` opens with the real user config (library `V:/OmniScan/data/raws`, series `PepperCarrotKR`), the Library and Reader pages show "Episode 06" aligned raw|output, and a Run in *Subset* mode with only `typeset`+`export` completes from the GUI. State in the report what you verified and how (offscreen is fine).
