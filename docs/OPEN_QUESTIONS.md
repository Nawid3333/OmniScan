...
| ID | Question | Blocks / why it matters | Default until answered |
|---|---|---|---|
| B1 | **Windows-native development vs WSL** — see the assessment in the chat/CHECKPOINT once the Windows GPU test has run | Where builders and I work day to day; whether the Qt app can be tested on the real target OS | Keep WSL as the reference environment; keep the code portable and add Windows/macOS CPU CI |
...
| B5 | **Containers inside the app** — you suggested it; my view: not for end users (Docker Desktop needs WSL2/Hyper-V and has **no Apple GPU passthrough**). Are you OK with native packaging instead? Docker stays as an optional server/dev path (B17) | Architecture of M13 | Native packaging, no embedded container |
