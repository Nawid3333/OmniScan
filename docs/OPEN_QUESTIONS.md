...
| F15 | `.github/workflows/ci.yml` rewritten 2026-09-24 to a single `windows-latest` job syncing `rocm-gfx1201` + `gui` (no OS matrix, no leftover `cpu` extra) — YAML validated, but **not yet verified by an actual GitHub Actions run** (nothing pushed yet, and it's unconfirmed whether `uv sync --extra rocm-gfx1201` can even reach/install from AMD's ROCm index on a GitHub-hosted runner with no AMD GPU). Push and watch the first run before trusting it green. | Whether CI is real or just looks right | Push when ready, check the Actions tab, fix forward if the sync step itself fails |
