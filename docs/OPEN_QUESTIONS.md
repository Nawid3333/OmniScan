...
| ~~F15~~ | ~~`.github/workflows/ci.yml` rewritten 2026-09-24 to a single `windows-latest` job syncing `rocm-gfx1201` + `gui`.~~ | — | **Resolved 2026-09-25: pushed (`66046868`) and verified. `uv sync --extra rocm-gfx1201 --extra gui` installs cleanly on a GitHub-hosted runner with no AMD GPU — that question is answered yes. The push's own first CI run showed red, but the failure was an unrelated pre-existing `test_model_watch_workflow` regression, not the sync step; that's since been fixed too, and `main`'s CI has been green on every push since.** |
...
| 2026-09-25 | Watermarks (E4) | Owner: "Filter the watermarks". Removed from the page by default (`inpaint.remove_watermarks`): ad/site text erased like lettering, stored fixed-position zones erased whole on every page; still never translated or lettered |
| 2026-09-25 | SFX coverage | Owner: effects must be replaced like their old EasyOCR pipeline did. The OCR stage now sweeps every page with CRAFT (EasyOCR's detector) for effects the comic detector misses (`sfx.sweep`, on) |
