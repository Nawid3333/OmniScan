...
| 2 | **C6a** | Inpaint v1: text mask from OCR line boxes (dilated), flat fill for uniform bubble interiors (ring-colour + variance test), everything else flagged `needs_lama`; `InpaintStage` → `inpaint.json` + `patches.npz` | contracts (D3), X1, C4a | T2/T3 | write card |
| 2 | **C7a** | Typeset layout engine (pure): fit a line of English into a box — font-size search, greedy line breaking with PIL font metrics, alignment, overflow flag → `LayoutArtifact` | contracts (D3), X1 | T1/T2 | write card |
