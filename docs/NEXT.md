...
| S5 | **F2a → F2b → F2c** | **F2a ✔ merged 2026-09-22** (tier 2: file-level in `ingest`, slice-level in `slice`, `filter.json` overrides, `filter add/run/restore/force` CLI; a live-check on the real GPU caught and the builder fixed a ROCm crash in `dhash_tensor` on full-page crops — `docs/reports/F2a.md`). F2b = tier 1 post-OCR text patterns (Discord/Patreon/"translated by"/URLs, global + per series, restorable); F2c = tier 3 position/shape heuristics + batch over all chapters | F2a card next |
...
| S8 | **U3b → U3a → U3c → U3e** | ✔ all merged | Desktop app (PySide6, native widgets): **U3b** strip view + side-by-side viewer, **U3a** models view, **U3c** main window (sidebar Library/Reader/Run/Models/Settings, `omniscan gui`, verified on the real PepperCarrotKR library), **U3e** import/upload page (archives, JPEG conversion, editable chapter grouping — director found and fixed a zip-slip vulnerability before merging, `docs/reports/U3e.md`) all merged 2026-09-22. Remaining: wire `ImportView` into the main window's sidebar (small connecting piece, unblocked now both are merged — not yet done, worth a tiny follow-up card) | done, one small follow-up open |
| S16 | **CM1** | ✔ merged 2026-09-22 | Chapter matching across two independently-sourced chapter sets: perceptual page hashing + two-level Needleman-Wunsch alignment (page level, chapter level), `omniscan match chapters DIR_A DIR_B`. 100–200 chapters × 20 pages timed at 2.5–8.0 s | done |
| S17 | **GL1** | ✔ merged 2026-09-22 | Reference-mode glossary bootstrap (decision D7): given raws + an official English translation for some chapters (imported under `<Series>/_reference_en/`), matches via CM1, OCRs both sides, extracts terms with an LLM, auto-locks a `(source, target)` pair recurring in ≥3 reference chapters. `omniscan reference SERIES` — no longer a stub. Verified on real GPU hardware. Known follow-up: the reference side is OCR'd with the series' source-language engine, not an English-tuned one — needs a `core/config.py` per-side override, out of this card's scope | done, one follow-up noted |
...
- **Ollama Cloud limits** — builders and cloud translation share a rolling ~5 h limit (3 concurrent requests) **and a weekly usage limit** (hit 2026-09-20 ~20:00, clear again by 2026-09-22 ~16:30 — under 2 days, so likely a rolling window, not a fixed weekly reset; re-check with `omni_builder.py smoke` before a big batch). F2a and G3 were resumed (`omni_builder.py resume <ID> <feedback file>`, needs the worktree's `.venv` junction recreated first if it was deleted).
- **fp16 is unreliable on this ROCm-Windows stack** (MIOpen); everything runs fp32. Re-test when torch/ROCm updates.
- **Builder sandboxing gap found and closed 2026-09-22**: `.builder/settings.json` granted blanket Read/Edit/Write with no path scoping beyond a few narrow deny patterns, so a builder session reached the director's live cross-session memory (`~/.claude/...`, pointed at by `docs/memory/README.md`) and edited two files there. Now denied (`~/.claude/**`) in both `.builder/settings.json` and `.builder/ask-settings.json`. If a future card needs a builder to touch something outside its worktree for a legitimate reason, that now needs an explicit, narrow allow — don't just widen the blanket grant back.
- **Dependabot: `setuptools` 81.0.0** (transitive, via AMD's ROCm wheel index) has an open moderate advisory (macOS-specific sdist-filtering bypass via NFC/NFD Unicode normalisation). Low real-world exposure here (Windows/Linux primary targets, setuptools is build-time only, never runs in the pipeline) — not yet bumped because it comes from a specialised index, not plain PyPI, so a version bump needs checking it doesn't destabilise the pinned torch/ROCm chain. Owner was informed, deferred as low-priority.
