...
MUTANTS = [
    ...
    (
        ...
        "return max(counts.items(), key=lambda item: (item[0], item[1]))[0]",
        "tie key width/count swapped",
    ),
    (
        LA,
        "return max(counts.items(), key=lambda item: (item[1], item[0]))[0]",
        "return max(counts.items(), key=lambda item: (item[1], -item[0]))[0]",
        "tie breaks to smaller width",
    ),
    (
        LA,
        "return max(counts.items(), key=lambda item: (item[1], item[0]))[0]",
    ...
    (
        LA,
        "return max(counts.items(), key=lambda item: (item[1], item[0]))[0]",
        "return max(counts.items(), key=lambda item: (item[1], item[0]))[1]",
        "return count not width",
    ),
    (
        LA,
    ...
    (
        ...
        "scaled_height = min(1, round(height * scale))",
        "max(1,) -> min(1,)",
    ),
    (
        LA,
    ...
    (
        ...
        "layout.append((scale, y, y + height))",
        "y1 uses raw height",
    ),
    ...
    (
        ...
        "drop .jpeg from suffix set",
    ),
    (
        CO,
        'if path.suffix.lower() not in (".jpg", ".jpeg"):\n        return True',
        'if path.suffix.lower() not in (".jpg", ".jpeg"):\n        return False',
        "suffix branch True -> False",
    ),
    (
        CO,
        'if img.mode in _ALPHA_MODES or (img.mode == "P" and "transparency" in img.info):',
        'if img.mode in _ALPHA_MODES and (img.mode == "P" and "transparency" in img.info):',
        "alpha or -> and",
    ),
    (
        CO,
        'if img.mode in _ALPHA_MODES or (img.mode == "P" and "transparency" in img.info):',
        'if img.mode in _ALPHA_MODES or (img.mode == "P" and "transparency" not in img.info):',
        "transparency in -> not in",
    ),
    ...
    (
        ...
        'dest = cache_dir / f"{src.stem}.jpg"',
        "dest name drops index",
    ),
    (
        CO,
        "sha256=hash_file(dest), width=out.width, height=out.height, converted=True",
        "sha256=hash_file(src), width=out.width, height=out.height, converted=True",
        "sha of src not dest",
    ),
    (
        CO,
        "width=out.width, height=out.height, converted=True",
        "height=out.width, width=out.height, converted=True",
        "converted width/height swapped",
    ),
    ...
    (
        ...
        "    dest = cache_dir",
        "drop cache_dir.mkdir",
    ),
    (
        CO,
        'out.save(dest, format="JPEG", quality=quality, subsampling=0, optimize=True)',
        'out.save(dest, format="PNG", quality=quality, subsampling=0, optimize=True)',
        "save as PNG",
    ),
]
