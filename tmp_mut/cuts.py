...
MUTANTS = [
    ...
    (C, "return lo + int(idx[dist.argmin()].item())", "return lo + int(idx[dist.argmax()].item())", "tie argmin -> argmax"),
    # ---- _dp_cuts
    ...
    (C, "_dp_cuts(a, b, cands, cfg.target_height, cfg.min_height, cfg.max_height, cfg.hard_max_height)",
    ...
    (C, "if length >= cfg.min_height + min_band:", "if length > cfg.min_height + min_band:", "edge/centre threshold >= -> >"),
    ...
    (C, "k = math.ceil(gap / cfg.max_height) - 1", "k = math.floor(gap / cfg.max_height) - 1", "ceil -> floor"),
    ...
    (C, "mandatory = sorted({0, height} | edges | set(forced))", "mandatory = sorted({0, height} | edges)", "drop forced from mandatory"),
    (C, "positions = [0, *sorted(centres | edges), height]", "positions = [*sorted(centres | edges), height]", "drop 0 from positions"),
    # ---- plan_cuts: flags / output
    (C, "for y in edges:\n        cuts[y] = False", "for y in edges:\n        cuts[y] = True", "edge cuts marked forced"),
    (C, "for y in forced:\n        cuts[y] = True", "for y in forced:\n        cuts[y] = False", "forced cuts marked normal"),
    (C, "cuts.setdefault(y, False)", "cuts.setdefault(y, True)", "dp cuts marked forced"),
    (C, "return [Cut(y=y, forced=cuts[y]) for y in sorted(cuts)]",
       "return [Cut(y=y, forced=cuts[y]) for y in sorted(cuts, reverse=True)]", "cuts sorted descending"),
]