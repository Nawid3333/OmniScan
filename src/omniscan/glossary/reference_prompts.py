...
EXTRACT_SYSTEM = (
    "You are building the glossary of an official English release of a Korean manhwa from its own "
    "translated chapters. You get one chapter's text as paired lines: 'source' is the Korean "
    "original, 'target' is the official English translation of that same line. List every term a "
    "glossary should bind: character names, place names, organisations, ranks and titles, recurring "
    "items, skills or techniques. For each term give the Korean source exactly as it appears in a "
    "source line with any attached particle (이, 가, 은, 는, 을, 를, 의, 도, 아, 야, 에, 에게, 에서) "
    "removed; the English target exactly as the paired target lines spell it; and one type: person, "
...
def terms_messages(lines: Sequence[PairedLine]) -> list[dict[str, str]]:
